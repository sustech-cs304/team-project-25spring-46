...
export default function HomePage() {
    ...
    return (<div className="flex flex-col items-center space-y-10">
      ...
      <div className="flex flex-col items-center space-y-6 w-full max-w-xl">
        ...
        {courses.map((course, index) => (<CourseCard key={index} title={course.name} progress={course.progress !== undefined ? course.progress : 0} ddl={course.ddl || "2025-04-30"}/>))}
      </div>
      <div className="mt-auto pb-10 flex gap-4">
        <button className="bg-blue-600 text-white px-6 py-3 rounded-xl text-lg" onClick={handleAddCourse}>
          ➕ 添加课程
        </button>
        <button className="bg-gray-300 px-6 py-3 rounded-xl text-lg" onClick={handleImportCourse}>
          📁 导入课程
        </button>
      </div>
    </div>);
