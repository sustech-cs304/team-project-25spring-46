...
export default function FilePage({ filePath }) {
    ...
    if (vscode == null) {
      ...
      <div className="flex flex-col items-center">
        ...
        <select value={selectedFile} onChange={(e) => setSelectedFile(e.target.value)} className="border p-2 rounded-md w-full max-w-sm">
      ...
      <div className="flex justify-center">
        <button className="bg-blue-600 text-white px-6 py-2 rounded-xl text-lg hover:bg-blue-700" onClick={() => console.log(`打开文件 ${selectedFile}`)}>
          🔍 查看文件
        </button>
      </div>
      ...
      <div className="bg-white shadow p-4 rounded-xl">
        ...
        <button className="bg-indigo-500 text-white px-4 py-2 rounded-lg hover:bg-indigo-600" onClick={() => setShowCode(!showCode)}>
          {showCode ? "🔽 收起识别代码" : "🧩 启动代码识别"}
        </button>
        {showCode && (<ul className="mt-4 list-disc list-inside text-sm text-gray-700">
      ...
      <div className="bg-white shadow p-6 rounded-xl">
        ...
        <div className="space-y-6">
          {comments
                ...
                .map((c, idx) => (<div key={idx} className="border-b pb-4">
                ...
                <div className="mt-2">
                  <input type="text" value={replyInput[idx] || ""} onChange={(e) => setReplyInput((prev) => ({ ...prev, [idx]: e.target.value }))} placeholder="写下你的回复..." className="w-full border rounded p-2 text-sm"/>
                  <button onClick={() => handleReplySubmit(idx)} className="mt-1 text-sm text-green-600 hover:underline">↩️ 回复</button>
                </div>
                {replies[idx] && replies[idx].length > 0 && (<ul className="mt-2 pl-4 list-disc text-sm text-gray-700 space-y-1">
                    {replies[idx].map((reply, ridx) => (<li key={ridx}>{reply}</li>))}
                  </ul>)}
              </div>))}
          {comments.filter((c) => c.page === selectedFile).length === 0 && (<p className="text-gray-500">暂无评论</p>)}
        </div>
      </div>
      ...
      <div className="bg-white shadow p-6 rounded-xl">
        ...
        <ul className="list-disc list-inside space-y-2 text-blue-600">
          {resources.map((r, idx) => (<li key={idx}>
    ...
    return (<div className="max-w-3xl mx-auto space-y-10">
      ...
      {fileDetails ? (<div className="text-center">
      ...
      <div className="flex justify-center">
        <button className="bg-blue-600 text-white px-6 py-2 rounded-xl text-lg hover:bg-blue-700" onClick={() => vscode && vscode.postMessage({ command: 'openFile', filePath })}>
          🔍 查看文件
        </button>
      </div>
      ...
      <div className="bg-white shadow p-4 rounded-xl">
  <h2 className="text-xl font-semibold mb-4">🧠 代码识别</h2>
  <button className="bg-indigo-500 text-white px-4 py-2 rounded-lg hover:bg-indigo-600" onClick={runCodeRecognition}>
    🚀 运行代码识别
      </button>

      {showCode && (<div className="mt-4">
          {codeFiles.length ? (codeFiles.map((code, idx) => {
                ...
                return (<div key={idx} className="bg-gray-100 rounded p-4 mb-2">
...
//# sourceMappingURL=FilePage.js.map